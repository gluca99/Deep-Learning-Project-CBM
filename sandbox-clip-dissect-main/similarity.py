...
def cos_similarity(clip_feats, target_feats):
    with torch.no_grad():
        ...
        for orig_id in tqdm(range(target_feats.shape[1])):
            curr_errors = []
            for i in range(math.ceil(len(clip_feats)/batch_size)):
                curr_errors.append(target_feats[:, orig_id:orig_id+1].T @ 
                                   clip_feats[:, i*batch_size:(i+1)*batch_size].float())
            errors.append(torch.cat(curr_errors, dim=1))
