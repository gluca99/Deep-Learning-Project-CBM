...
solver=grad
gamma=0.2
dropout_rate=0 # set to 0 if no dropout should be applied

SAVE_DIR=~/result/seed${SEED}/nopretrain_dropout
...
# uncomment these two lines if running with random seed
concept_path=~/${ROOT}/sandbox-lf-cbm/data/concept_sets/separated_concepts/${dataset}_task_${i}_${SEED}_filtered.txt
python ~/${ROOT}/sandbox-lf-cbm/train_cbm.py --nonlinear ${nonlinear} --seed ${SEED} --gamma ${gamma} --solver ${solver} --lam ${spar_lam} --dropout_rate ${dropout_rate} --concept_method ${con_method} --dataset ${DATASET} --concept_set ${concept_path} --save_dir ${SAVE_DIR} --freeze_wc ${freeze} --normalize_wf ${nor_wf} --proj_steps ${PROJ_STEPS} --proj_batch_size ${PROJ_BATCH_SIZE} --val_log_interval ${VAL_LOG_INTERVAL} --interpretable --extend_concept_set

# uncomment these two lines if running with fixed data set
#concept_path=~/${ROOT}/sandbox-lf-cbm/data/concept_sets/separated_concepts/our_separated_concepts/${dataset}_task_${i}_${SEED}.txt
#python ~/${ROOT}/sandbox-lf-cbm/train_cbm.py --nonlinear ${nonlinear} --seed fix --gamma ${gamma} --solver ${solver} --lam ${spar_lam} --concept_method ${con_method} --dataset ${DATASET} --concept_set ${concept_path} --save_dir ${SAVE_DIR} --freeze_wc ${freeze} --normalize_wf ${nor_wf} --proj_steps ${PROJ_STEPS} --proj_batch_size ${PROJ_BATCH_SIZE} --val_log_interval ${VAL_LOG_INTERVAL} --interpretable --extend_concept_set
done
